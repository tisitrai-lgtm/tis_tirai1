...
$sql = "SELECT * FROM image_water WHERE plot_id = ?";
$stmt = mysqli_prepare($con, $sql);
mysqli_stmt_bind_param($stmt, "s", $plot_id);
    ...
    <div class="card-section">
      ...
      <div class="photo-grid">
        ...
        foreach ($images as $name => $meta) {
          ...
          echo '  </label>';
          if ($has_img) {
            echo "  <a href=\"javascript:void(0);\" class=\"btn btn-delete-img\" onclick=\"confirmDelete('{$name_js}', '{$plot_id_js}')\"><i class=\"bi bi-trash\"></i> ลบรูปภาพนี้</a>";
          }
          echo '</div>';
        }
        ?>
...
<script>
  ...
  function confirmDelete(imageName, plotId) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = `user_waterdelete.php?delete_image=${imageName}&plot_id=${plotId}`;
      }
    });
  }
</script>

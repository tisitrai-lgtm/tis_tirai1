    ...
    <div class="container">
        ...
        <?php if (!empty($selected_year)): ?>
            ...
            ?>
                ...
                <?php
                // แก้ไข: ลบ 'notes' ใน SELECT statement
                $stmt_plots = $conn->prepare("SELECT plot_id, rai_area FROM soil_data WHERE production_year = ? AND agency = ? AND contract_number = ? ORDER BY plot_id");
                ...
                if ($result_plots->num_rows > 0) {
                    ...
                    // แก้ไข: ลบ <th>หมายเหตุ</th> ออก
                    echo "<thead><tr><th>ID แปลง</th><th>พื้นที่ (ไร่)</th><th>ดูรายละเอียดแปลง</th></tr></thead>";
                    echo "<tbody>";
                    while ($row_plot = $result_plots->fetch_assoc()) {
                        ...
                        // แก้ไข: ลบส่วนของโค้ดที่แสดงหมายเหตุออก
    ...
    $(document).ready(function() {
        ...
        if ($('#plotListTable').length) {
            $('#plotListTable').DataTable({
                ...
                "columnDefs": [
                    // แก้ไข: หมายเหตุถูกลบออกไปแล้ว ทำให้ "ดูรายละเอียดแปลง" กลายเป็น targets index 2
                    { "orderable": false, "targets": [2] } // Disable sorting on "ดูรายละเอียดแปลง"
                ],
                "paging": true, // Enable pagination
                "info": true,   // Enable info
                "lengthMenu": [[10, 20, 50, 100, -1], [10, 20, 50, 100, "All"]], // Show entries options
        ...
        // แก้ไข: ลบ JavaScript สำหรับ Read More/Read Less ออก เนื่องจากไม่มีคอลัมน์หมายเหตุแล้ว
    });
    </script>
</body>
</html>

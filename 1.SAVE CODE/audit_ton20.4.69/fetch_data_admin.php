...
// รับค่าปีการผลิตที่ส่งมาจาก index.php
$selected_year = $_POST['year'] ?? '';
// 🚨 โค้ดที่เพิ่ม: รับค่าตัวกรองสถานะรูปภาพ
$image_status_filter = $_POST['image_status_filter'] ?? ''; 
...
// ------------------------------------------------------------------
// --- การตั้งค่าสำหรับตัวกรองรูปภาพ ---
// ------------------------------------------------------------------

// รายการคอลัมน์รูปภาพทั้งหมด
$image_fields = [
...
// สร้างเงื่อนไข SQL สำหรับตรวจสอบว่ามีรูปภาพอย่างน้อย 1 รูปหรือไม่ (โดยใช้ != '' และ IS NOT NULL เพื่อความครอบคลุม)
// เนื่องจาก field เป็น VARCHAR และเก็บชื่อไฟล์ มักจะถูกบันทึกเป็น empty string ('')
$image_conditions = [];
foreach ($image_fields as $field) {
    // ตรวจสอบว่าไม่ใช่ค่าว่าง (empty string) และไม่ใช่ NULL
...
// ------------------------------------------------------------------
// --- 2. สร้าง SQL Query สำหรับดึงข้อมูล (พร้อมการค้นหาและกรองปี/รูปภาพ) ---
// ------------------------------------------------------------------
$sql = "SELECT * FROM cane_plot_data WHERE 1=1";
$params = [];
$types = "";

// เงื่อนไข: กรองตามปีการผลิต
if (!empty($selected_year)) {
...
// 🚨 โค้ดที่เพิ่ม: เงื่อนไข: กรองตามสถานะรูปภาพ
if ($image_status_filter === 'has_image') {
    // กรองเฉพาะแปลงที่มีรูปภาพ
    $sql .= " AND {$image_check_sql}";
} elseif ($image_status_filter === 'no_image') {
    // กรองเฉพาะแปลงที่ไม่มีรูปภาพ
    $sql .= " AND NOT {$image_check_sql}";
}

// เงื่อนไข: กรองตามคำค้นหา
...
if ($image_status_filter === 'has_image') {
    // กรองเฉพาะแปลงที่มีรูปภาพ
    $filteredRecordsSql .= " AND {$image_check_sql}";
} elseif ($image_status_filter === 'no_image') {
    // กรองเฉพาะแปลงที่ไม่มีรูปภาพ
    $filteredRecordsSql .= " AND NOT {$image_check_sql}";
}

...
<head>
    ...
    <style>
        ...
        .nav-admin-group { position: relative; display: flex; align-items: center; }
        .nav-admin-toggle {
        ...
        .nav-admin-toggle:hover, .nav-admin-toggle.active { color: white; }
        .nav-admin-toggle .arrow-icon { font-size: 0.7rem; transition: transform 0.2s; }
        .nav-admin-group.open .arrow-icon { transform: rotate(180deg); }

        .nav-admin-dropdown {
            display: none; position: absolute; top: calc(100% + 14px); left: 0;
            background: #1e3a5f; border-radius: 10px; min-width: 210px;
            box-shadow: 0 8px 24px rgba(0,0,0,0.25); border: 1px solid rgba(255,255,255,0.08);
            overflow: hidden; z-index: 1001;
        }
        .nav-admin-group.open .nav-admin-dropdown { display: block; animation: fadeInNoti 0.18s ease-out; }

        /* หัวข้อกั้นในดรอปดาวน์ */
        .nav-dropdown-label {
            font-size: 0.68rem; font-weight: 700; letter-spacing: 0.09em;
            text-transform: uppercase; color: #64748b;
            padding: 10px 14px 4px; display: block;
        }
        .nav-dropdown-divider { border: none; border-top: 1px solid rgba(255,255,255,0.07); margin: 4px 0; }

        /* แต่ละ item ในดรอปดาวน์ */
        .nav-dropdown-item {
        ...
        .nav-dropdown-item.active { background: rgba(16,185,129,0.12); color: #34d399; }
        ...
        .nav-dropdown-item.active .item-icon { background: rgba(16,185,129,0.15); color: #34d399; }
        ...
        @media (max-width: 991px) {
            ...
            /* Admin group — mobile: แสดงเป็น flat list มีหัวกั้น */
            .nav-admin-group { flex-direction: column; align-items: flex-start; width: 100%; }
            .nav-admin-toggle { display: none; }
            .nav-admin-dropdown {
                display: block !important; position: static;
                background: transparent; box-shadow: none; border: none;
                border-radius: 0; min-width: unset; width: 100%;
                padding: 0;
            }
            .nav-dropdown-label {
                color: #64748b; padding: 6px 0 2px;
                border-left: 3px solid #e11d48; padding-left: 8px;
            }
            .nav-dropdown-item {
                padding: 8px 0 8px 8px; font-size: 1rem;
                border-bottom: 1px solid rgba(255,255,255,0.05);
                width: 100%;
            }
            .nav-dropdown-item .item-icon { display: none; }
...
<nav class="main-navbar">
    ...
    <div class="nav-menu" id="navMenu">
        ...
        <?php
        $admin_pages = ['harvester_admin.php', 'manage_users.php', 'setting_system.php'];
        $is_admin_page_active = in_array($current_page, $admin_pages);
        ?>

        <?php if(isset($_SESSION['emp_level']) && $_SESSION['emp_level'] == 'a'): ?>
            <!-- Admin Dropdown Group -->
            <div class="nav-admin-group <?php echo $is_admin_page_active ? 'open' : ''; ?>" id="adminNavGroup">
                <button type="button" class="nav-admin-toggle <?php echo $is_admin_page_active ? 'active' : ''; ?>" onclick="toggleAdminMenu(event)">
                    <i class="fa-solid fa-shield-halved"></i> จัดการ
                    <i class="fa-solid fa-chevron-down arrow-icon"></i>
                </button>
                <div class="nav-admin-dropdown">
                    <span class="nav-dropdown-label">จัดการระบบ</span>
                    <a href="harvester_admin.php" class="nav-dropdown-item <?php echo ($current_page == 'harvester_admin.php') ? 'active' : ''; ?>">
                        <span class="item-icon"><i class="fa-solid fa-chart-bar"></i></span>
                        จัดการตรวจเช็กรถตัด
                    </a>
                    <a href="manage_users.php" class="nav-dropdown-item <?php echo ($current_page == 'manage_users.php') ? 'active' : ''; ?>">
                        <span class="item-icon"><i class="fa-solid fa-users-gear"></i></span>
                        จัดการผู้ใช้
                    </a>
                    ...
                    <a href="setting_system.php" class="nav-dropdown-item <?php echo ($current_page == 'setting_system.php') ? 'active' : ''; ?>">
                        <span class="item-icon"><i class="fa-solid fa-gear"></i></span>
                        ตั้งค่าระบบ
                    </a>
                </div>
            </div>
...
// เปิด-ปิด Admin Dropdown
function toggleAdminMenu(event) {
    event.stopPropagation();
    var group = document.getElementById("adminNavGroup");
    group.classList.toggle("open");
}

// คลิกข้างนอก Admin Dropdown ให้ปิด

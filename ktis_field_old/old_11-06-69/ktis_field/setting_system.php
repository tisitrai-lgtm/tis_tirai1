...
<title>ตั้งค่าระบบ - KTIS SMART FIELD</title>
            ...
            <div class="list-item" id="cut-<?php echo $c['item_id']; ?>">
                ...
                <div style="display:flex;gap:6px;">
                    <button class="btn-del" style="border-color:#bfdbfe;color:#3b82f6;"
                            ...
                            onclick="editCutItem(<?php echo $c['item_id']; ?>, '<?php echo addslashes(htmlspecialchars($c['item_name_cut'])); ?>')">
...
<div class="card">
            ...
            <div class="list-item" id="field-<?php echo $f['item_id']; ?>">
                ...
                <div style="display:flex;gap:6px;">
                    <button class="btn-del" style="border-color:#fde68a;color:#d97706;"
                            ...
                            onclick="editFieldItem(<?php echo $f['item_id']; ?>, '<?php echo addslashes(htmlspecialchars($f['item_name_field'])); ?>')">
                        <i class="fa-solid fa-pen"></i> แก้ไข
                    </button>
    ...
    <div class="card card-full">
        <div class="card-hd purple">
            <div class="card-hd-l"><i class="fa-solid fa-clock-rotate-left"></i><span>ประวัติการทำงาน (System Logs)</span></div>
            <span class="cnt-badge">50 รายการล่าสุด</span>
        </div>
        <div class="card-bd" style="padding:0;">
            <?php if(empty($logs)): ?>
            <div class="empty-list" style="padding:32px;"><i class="fa-solid fa-inbox" style="font-size:2rem;display:block;margin-bottom:8px;"></i>ยังไม่มีประวัติ</div>
            <?php else: ?>
            <div class="log-wrap">
                <table>
                    <thead>
                    ...
                    <?php foreach($logs as $log):
                        ...
                        if(strpos($log['action_type'],'CREATE') !== false) $badge_class = 'badge-create';
                        elseif(strpos($log['action_type'],'DELETE') !== false) $badge_class = 'badge-delete';
                        elseif(strpos($log['action_type'],'EDIT') !== false || strpos($log['action_type'],'CHANGE') !== false) $badge_class = 'badge-edit';
                        elseif(strpos($log['action_type'],'STATUS') !== false) $badge_class = 'badge-status';
                    ?>
                    <tr>
                        <td>
                            <div style="font-weight:700;font-size:.8rem;color:#1e293b;"><?php echo date('d M Y', strtotime($log['created_at'])); ?></div>
                            <div style="font-size:.73rem;color:#94a3b8;"><?php echo date('H:i น.', strtotime($log['created_at'])); ?></div>
                        </td>
                        <td style="font-weight:700;color:#1e293b;font-size:.83rem;"><?php echo htmlspecialchars($log['emp_name'] ?? $log['action_by']); ?></td>
                        <td><span class="action-badge <?php echo $badge_class; ?>"><?php echo htmlspecialchars($log['action_type']); ?></span></td>
                        <td style="font-family:monospace;font-size:.8rem;color:#64748b;"><?php echo htmlspecialchars($log['target_id'] ?? '-'); ?></td>
                        <td style="color:#475569;font-size:.82rem;max-width:300px;"><?php echo htmlspecialchars($log['log_details']); ?></td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>

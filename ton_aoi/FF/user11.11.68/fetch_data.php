...
$filter_image = $_POST['filter_image'] ?? 0; // 🚨 NEW: รับค่าตัวกรองรูปภาพ (0=ไม่กรอง, 1=กรอง)
...
// 🚨 เงื่อนไข SQL สำหรับแปลงที่มีรูปภาพอย่างน้อย 1 รูป (เช็คทั้ง 10 คอลัมน์)
...
if (!empty($selected_year) && !empty($selected_agency)) {
    ...
    $params[] = $selected_agency; // 🚨 เพิ่ม Agency ในพารามิเตอร์
    $types .= "ss"; // s สำหรับ year, s สำหรับ agency
}

// 🚨 NEW: เพิ่มเงื่อนไขกรองรูปภาพ
if ($filter_image == 1) { 
    $sql .= " AND " . $image_filter_condition;
}
...
if ($filter_image == 1) { 
    $filteredRecordsSql .= " AND " . $image_filter_condition;
}
...
if (!empty($finalParams)) {
    ...
    if (strlen($finalTypes) != count($finalParams)) {
        // กรณีผิดพลาดในการ Bind (ไม่น่าจะเกิดขึ้นถ้าโค้ดถูกต้อง)
        // เพื่อความปลอดภัย เราจะไม่ส่งข้อมูล
...
while ($row = $result->fetch_assoc()) {
    ...
    // 🚨 สร้าง URL เต็มสำหรับรูปภาพ 10 คอลัมน์ (ตามโครงสร้างโฟลเดอร์ที่ให้มา)

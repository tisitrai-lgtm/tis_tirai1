<?php
// user_edit_data_evaluate.php - ฟอร์มแก้ไขข้อมูลการประเมิน
 ...
 */
function createImageUploadsEdit($title, $images, $currentData, $basePath, $recordId, $iconClass = 'bi-bar-chart-line-fill') {
...
<head>
    ...
    <title>แก้ไขข้อมูลประเมินตันอ้อย: <?php echo htmlspecialchars($data['plot_id']); ?></title>
...
<div class="container mt-5">
    <div class="main-card">
        ...
        <form id="editForm" action="user_update_data.php" method="POST" enctype="multipart/form-data">
            ...
            <h4 class="section-title">
                <i class="bi bi-pencil-square me-2"></i> ข้อมูลรูปภาพประเมินตันอ้อย
            </h4>
            
            <?php
            $section1_images = [
                "evaluate_ton_1" => ["label" => "ประเมินตันอ้อย รูปหมุด (1/2)", "subfolder" => "evaluate_ton"],
                "evaluate_ton_2" => ["label" => "ประเมินตันอ้อย รูปอ้อย (2/2)", "subfolder" => "evaluate_ton"],
            ];
            // ใช้ bi-bar-chart-line-fill แทน bi-camera-fill เพื่อให้สื่อถึง "การประเมิน"
            createImageUploadsEdit("รูปภาพการประเมินตันอ้อยหลังเปิดหีบ", $section1_images, $data, $basePath, $data['id'], 'bi-bar-chart-line-fill');
            ?>
            ...
            <div class="mt-4 mb-3 text-center">
                ...
                <a href="user_dashboard_evaluate.php?year=<?php echo htmlspecialchars($selected_year); ?>&agency=<?php echo htmlspecialchars($selected_agency); ?>" class="btn btn-secondary btn-lg shadow-sm" style="min-width: 180px;">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // 4. Redirect ไปยัง user_dashboard_evaluate.php (ตามที่โค้ดเดิมกำหนด)
                    var redirectUrl = 'user_dashboard_evaluate.php?year=' + encodeURIComponent(response.year) + 

<?php
// user_edit_data_remaining.php - ฟอร์มแก้ไขข้อมูลรูปภาพที่เหลือ
 ...
 * @param string $iconClass คลาสไอคอน Bootstrap
 */
function createImageUploadsEdit($title, $images, $currentData, $basePath, $recordId, $iconClass = 'bi-layers-fill') {
    ...
    foreach ($images as $name => $details) {
        ...
        // 🚨 แก้ไขให้ใช้ชื่อฟิลด์ใน DB ที่ถูกต้อง (เช่น remaining_1_1, remaining_1_2, ...)
        // เนื่องจากโค้ดเดิมใช้ "remaining_1" และ "remaining_2" ซึ่งไม่ถูกต้องสำหรับ 6 รูป
...
<head>
    ...
    <title>แก้ไขข้อมูลรูปภาพส่วนที่เหลือ: <?php echo htmlspecialchars($data['plot_id']); ?></title>
...
<div class="container mt-5">
    <div class="main-card">
        ...
        <form id="editForm" action="user_update_data.php" method="POST" enctype="multipart/form-data">
            ...
            <h4 class="section-title">
                <i class="bi bi-layers-fill me-2"></i> ข้อมูลรูปภาพส่วนที่เหลือ (รวม 6 รูป)
            </h4>
            
            <?php
            // ส่วนที่ 1: อ้อยคงเหลือ 1 (2 รูป)
            $section1_images = [
                "remaining_cane_1_img_1" => ["label" => "อ้อยคงเหลือ 1 รูปหมุด (1/2)", "subfolder" => "remaining_cane_1"],
                "remaining_cane_1_img_2" => ["label" => "อ้อยคงเหลือ 1 รูปอ้อย (2/2)", "subfolder" => "remaining_cane_1"],
            ];
            createImageUploadsEdit("1. รูปภาพอ้อยคงเหลือส่วนที่ 1", $section1_images, $data, $basePath, $data['id'], 'bi-1-circle-fill');

            // ส่วนที่ 2: อ้อยคงเหลือ 2 (2 รูป)
            $section2_images = [
                "remaining_cane_2_img_1" => ["label" => "อ้อยคงเหลือ 2 รูปหมุด (1/2)", "subfolder" => "remaining_cane_2"],
                "remaining_cane_2_img_2" => ["label" => "อ้อยคงเหลือ 2 รูปอ้อย (2/2)", "subfolder" => "remaining_cane_2"],
            ];
            createImageUploadsEdit("2. รูปภาพอ้อยคงเหลือส่วนที่ 2", $section2_images, $data, $basePath, $data['id'], 'bi-2-circle-fill');

            // ส่วนที่ 3: อ้อยคงเหลือ 3 (2 รูป)
            $section3_images = [
                "remaining_cane_3_img_1" => ["label" => "อ้อยคงเหลือ 3 รูปหมุด (1/2)", "subfolder" => "remaining_cane_3"],
                "remaining_cane_3_img_2" => ["label" => "อ้อยคงเหลือ 3 รูปอ้อย (2/2)", "subfolder" => "remaining_cane_3"],
            ];
            createImageUploadsEdit("3. รูปภาพอ้อยคงเหลือส่วนที่ 3", $section3_images, $data, $basePath, $data['id'], 'bi-3-circle-fill');
            ?>
            ...
            <div class="mt-4 mb-3 text-center">
                ...
                <a href="user_dashboard_remaining.php?year=<?php echo htmlspecialchars($selected_year); ?>&agency=<?php echo htmlspecialchars($selected_agency); ?>" class="btn btn-secondary btn-lg shadow-sm" style="min-width: 180px;">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // 4. Redirect ไปยัง user_dashboard_remaining.php (ตามที่โค้ดเดิมกำหนด)
                    var redirectUrl = 'user_dashboard_remaining.php?year=' + encodeURIComponent(response.year) + 

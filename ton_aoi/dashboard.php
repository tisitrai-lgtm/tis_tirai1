        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    const detailBtn = `<button class="btn btn-info btn-sm view-details-btn me-1"><i class='bx bx-search'></i> รายละเอียด</button>`;
                    const editBtn = `<a href="edit_data.php?plot_id=${plotId}" class="btn btn-warning btn-sm me-1"><i class='bx bx-edit'></i> แก้ไข</a>`;
                    // เพิ่มปุ่มลบ (ส่ง Primary Key 'id' และ 'plot_id' สำหรับข้อความยืนยัน)
                    const deleteBtn = `<button class="btn btn-danger btn-sm delete-btn" data-id="${dbId}" data-plot-id="${plotId}"><i class='bx bx-trash'></i> ลบ</button>`;

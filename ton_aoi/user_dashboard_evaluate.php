...
<head>
    ...
    <title>ระบบรูปใส่แปลงประเมินตัน</title>
    ...
    <style>
        ...
        .nav-link:hover { 
            color: #ffd700 !important; transform: translateY(-2px); 
        }
        ...
        /* สไตล์สำหรับ Dashboard Cards (การคลิกและการเลือก) */
        .summary-card {
            cursor: pointer; 
...
<div class="container">
    ...
    <h5 class="text-center">
        <i class='bx bx-search-alt'></i> ข้อมูลแปลงอ้อย ประเมินตันอ้อย (Evaluate)
    </h5>
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4">
            <div class="card text-white bg-success mb-3 shadow summary-card" data-filter-type="has_image">
                <div class="card-body">
                    <h5 class="card-title"><i class='bx bx-check-square'></i> แปลงที่มีรูปภาพ (Evaluate)</h5>
        ...
        <div class="col-md-4">
            <div class="card text-white bg-danger mb-3 shadow summary-card" data-filter-type="no_image">
                <div class="card-body">
                    <h5 class="card-title"><i class='bx bx-x-octagon'></i> แปลงที่ยังไม่มีรูปภาพ (Evaluate)</h5>
        ...
        <table id="dataTable" class="table table-striped table-bordered nowrap" style="width:100%">
            <thead>
                <tr>
                    ...
                    <th>พื้นที่ (ไร่)</th>
                    <th>ชนิดอ้อย</th>
                    <th>ประเมินตัน รูปหมุด</th>
                    <th>ประเมินตัน รูปอ้อย</th>
                    <th>หมายเหตุ</th>
                    <th>แก้ไข</th>
                </tr>
            </thead>
...
// 🚨 NEW FUNCTION: ดึงข้อมูลสรุป Dashboard (เปลี่ยนชื่อไฟล์ PHP)
function fetchSummaryCounts() {
    $.ajax({
        // 🚨 ต้องสร้าง fetch_summary_counts_evaluate.php ขึ้นมาใหม่
        url: 'fetch_summary_counts.php', // ใช้ชื่อเดิมไปก่อน และส่ง data_type ไป
        type: 'POST',
        dataType: 'json',
        data: {
...
$(document).ready(function () {
    ...
    const table = $('#dataTable').DataTable({
        ...
        columns: [
            ...
            { data: 'rai_area' },
            { data: 'suga_type' },
            // 🚨 เปลี่ยนคอลัมน์ที่ DataTables ดึงมาแสดง
            ...
            { data: 'notes', render: (data) => data && data.length > 25 ? `<span>${data.substring(0,25)}...</span> <a href="#" class="view-notes-btn" data-notes="${encodeURIComponent(data)}">ดูเพิ่มเติม</a>` : data },
            {
                ...
                render: (data, type, row) => `<a href="user_edit_data_evaluate.php?plot_id=${row.plot_id}" class="btn btn-warning btn-sm"><i class='bx bx-edit'></i> แก้ไข</a>`
            },

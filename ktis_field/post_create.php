...
// ── Validate ──
if (!$target_unit || !$truck_number || !$harvester_number || !$problem_1) {
    echo json_encode(['status' => 'error', 'message' => 'กรุณากรอกข้อมูลให้ครบ (หน่วย, ทะเบียน, รถตัด, ปัญหาที่ 1)']);
    exit;
}
...
try {
    $sql = "INSERT INTO posts 
                (emp_id, target_unit, truck_number, harvester_number, post_text,
    ...
    $noti_text  = "แจ้งรถสกปรก: {$truck_number} ({$harvester_number}) — {$prob_label}";
    ...
    $conn->prepare(
        ...
        ':by'  => $emp_id,
        ':tid' => $new_post_id,
        ':det' => "สร้างโพสต์ #{$new_post_id} หน่วย:{$target_unit} รถ:{$truck_number} รถตัด:{$harvester_number} ปัญหา:{$prob_label}",
    ]);

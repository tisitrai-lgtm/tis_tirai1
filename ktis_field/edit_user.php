...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // รับค่าจาก Checkbox (ถ้าติ๊กส่งมาเป็น '1' ถ้าไม่ติ๊กจะเป็น null)
        ...
        try {
            ...
            if ($check_result['count'] > 0) {
                ...
                if (!empty($emp_pass)) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE employee SET emp_name = ?, emp_id = ?, emp_unit = ?, emp_level = ?, emp_pass = ?, is_harvester_manager = ? WHERE ID = ?");
                    $update_stmt->execute([$emp_name, $emp_id_new, $emp_unit, $emp_level, $hashed_pass, $is_manager, $emp_id]);
                } else {
                    // ไม่เปลี่ยนรหัสผ่าน
                    $update_stmt = $conn->prepare("UPDATE employee SET emp_name = ?, emp_id = ?, emp_unit = ?, emp_level = ?, is_harvester_manager = ? WHERE ID = ?");
                    $update_stmt->execute([$emp_name, $emp_id_new, $emp_unit, $emp_level, $is_manager, $emp_id]);
                }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .toggle-container { display: flex; align-items: center; gap: 10px; margin-top: 10px; padding: 10px; background: #f1f5f9; border-radius: 8px; }
        .toggle-container input { width: 20px !important; height: 20px; cursor: pointer; }
       ...
       <div class="form-group">
            ...
            <select id="emp_unit" name="emp_unit" required>
                ...
                <?php foreach($zones as $z):
                    // ใช้รูปแบบเดียวกันทุกโซน (รหัส + เว้นวรรค + ชื่อ) เพื่อไม่ให้ค่าเพี้ยน
                    $unit_val = $z['zone_id'] . ' ' . $z['zone_name'];
                    
                    // เช็คว่าตรงกับข้อมูลเดิมของพนักงานไหม (รองรับทั้งแบบมีและไม่มีรหัส 000 เผื่อข้อมูลเก่าค้างอยู่)
                    $selected = ($employee['emp_unit'] === $unit_val || $employee['emp_unit'] === $z['zone_name']) ? 'selected' : '';
                ?>
                <option value="<?php echo htmlspecialchars($unit_val); ?>" <?php echo $selected; ?>>
                    <?php echo htmlspecialchars($z['zone_id'] . ' : ' . $z['zone_name']); ?>
                </option>
                <?php endforeach; ?>
            </select>
...
<div class="form-group">
    <label class="form-label">สถานะผู้ดูแลรถตัด:</label>
    <div class="toggle-container">
        <input type="checkbox" name="is_harvester_manager" id="is_harvester_manager" 
               <?php echo ($employee['is_harvester_manager'] == 1) ? 'checked' : ''; ?> value="1">
        <span class="toggle-label">กำหนดให้เป็นผู้ดูแลรถตัด</span>
    </div>
...
<script>
function updateManagerStatus(emp_id, isChecked) {
    const status = isChecked ? 1 : 0;
    fetch('update_manager_status.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: `emp_id=${emp_id}&status=${status}`
    }).then(res => res.json())
      .then(data => {
          if(data.success) {
              Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'อัปเดตสถานะผู้ดูแลแล้ว', showConfirmButton: false, timer: 1500 });
          } else {

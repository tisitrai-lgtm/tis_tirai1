 ...
 * @var array $_SESSION
 */
if (!function_exists('get_avatar_name')) {
...
<?php endif; ?>
        <?php if(empty($posts)): ?>
            <div class="empty-feed-card">
                ...
                <h4 class="empty-title">ไม่มีรายการแจ้งเหตุ</h4>
                <p class="empty-subtitle">ไม่พบรายการแจ้งเหตุรถอ้อยในวันที่เลือก หรือตามเงื่อนไขการค้นหา</p>
            </div>
        <?php else:
        ...
        ?>
            <?php foreach($posts as $post): 
                $is_success = ($post['job_status'] === 'success');
                $accent_color = $is_success ? '#10b981' : '#e11d48';
            ?>
                <div class="feed-card" id="post-card-<?php echo $post['post_id']; ?>" style="border-left: 5px solid <?php echo $accent_color; ?>;">
                    
                    <div class="post-header">
                        <div class="post-user-info">
                            <div class="post-user-avatar">
                                <?php echo htmlspecialchars(get_avatar_name($post['emp_name'])); ?>
                            </div>
                            <div>
                                <div class="post-user-name">
                                    <?php echo htmlspecialchars($post['emp_name']); ?>
                                    <span class="post-user-role">(ออฟฟิศกลาง)</span>
                                </div>
                                <div class="post-time">
                                    <i class="fa-regular fa-clock"></i> <?php echo date('H:i น.', strtotime($post['created_at'])); ?>
                                </div>
                            </div>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                            <span class="status-badge-chip <?php echo $is_success ? 'success' : 'pending'; ?>" data-status-badge>
                                <i class="fa-solid <?php echo $is_success ? 'fa-circle-check' : 'fa-clock'; ?>"></i>
                                <?php echo $is_success ? 'ดำเนินการแล้ว' : 'รอดำเนินการ'; ?>
                            </span>
                            <?php if($_SESSION['emp_level'] == 'a'): ?>
                                <button class="btn-delete-post" onclick="deletePost(<?php echo $post['post_id']; ?>)" title="ลบรายการนี้">
                                    <i class="fa-solid fa-trash-can"></i>
                                </button>
                                <button onclick="toggleStatus(<?php echo $post['post_id']; ?>, '<?php echo $post['job_status']; ?>')"
                                    title="<?php echo $is_success ? 'กดเพื่อเปลี่ยนกลับเป็นรอดำเนินการ' : 'กดเพื่อยืนยันดำเนินการแล้ว'; ?>"
                                    class="btn-status-toggle <?php echo $is_success ? 'btn-toggle-revert' : 'btn-toggle-confirm'; ?>"
                    ...
                    <div class="post-meta-badges">
                        ...
                        <span class="meta-badge"><i class="fa-solid fa-tractor" style="color:#06b6d4;"></i> รถตัด: <?php echo htmlspecialchars($post['harvester_number']); ?></span>
                        <?php endif; ?>
                    </div>

                    <div class="problem-box">
                        <div style="color:#991b1b; font-size:0.88rem; font-weight:800; display:flex; align-items:center; gap:6px;">
                            <i class="fa-solid fa-triangle-exclamation"></i> ปัญหาที่พบ:
                        </div>
                        <div style="margin:8px 0 0 0; display:flex; flex-wrap:wrap; gap:6px;">
                    ...
                    <?php if(!empty($post['post_text'])): ?>
                        <p class="post-main-text"><?php echo nl2br(htmlspecialchars($post['post_text'])); ?></p>
                    <?php endif; ?>
                    ...
                    <div class="reaction-bar">
                        <?php foreach($emo_map as $type=>[$icon,$label,$cls]): 
                            $is_my_react = ($my_rc === $type);
                        ?>
                        <button onclick="doReaction(<?php echo $post['post_id']; ?>,'<?php echo $type; ?>')"
                                id="rbtn-<?php echo $post['post_id']; ?>-<?php echo $type; ?>"
                                class="btn-reaction <?php echo $is_my_react ? 'active' : ''; ?>">
                            <span><?php echo $icon; ?></span> 
                            <span><?php echo $label; ?></span>
                            <span id="rcnt-<?php echo $post['post_id']; ?>-<?php echo $type; ?>" class="react-count"><?php echo $rc[$type]>0?$rc[$type]:''; ?></span>
                        </button>
                        <?php endforeach; ?>
                        
                        <?php if($total_rc>0): ?>
                        <span id="rtotal-<?php echo $post['post_id']; ?>" class="react-total-text"><?php echo $total_rc; ?> คน</span>
                        <?php else: ?>
                        <span id="rtotal-<?php echo $post['post_id']; ?>" class="react-total-text" style="display:none;"></span>
                        <?php endif; ?>
                        
                        <!-- ปุ่มไปหน้า detail -->
                        <a href="post_detail.php?id=<?php echo $post['post_id']; ?>" class="btn-comment-link">
                            <i class="fa-solid fa-comments"></i> 
                            <span>คอมเมนต์</span>
                            <span class="comment-count-chip"><?php
...
<?php if (!$is_ajax): ?>
    ...
    <div class="load-more-wrapper" style="text-align: center; margin: 25px 0;">
        <button id="btn-load-more" class="btn-load-more" onclick="loadMorePosts()">

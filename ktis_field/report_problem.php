...
<title>แจ้งปัญหา - TIS SMART FIELD</title>
        ...
        <div class="rp-item">
            ...
            <div class="rp-body">
                ...
                <div class="admin-note-box">
                    <i class="fa-solid fa-reply"></i> <strong>การตอบกลับจากผู้ดูแล:</strong> <?php echo htmlspecialchars($r['admin_note']); ?>
                </div>
                <?php endif; ?>
            </div>
            <?php if(!empty($r['img_path'])): ?>
            <img class="rp-thumb" src="<?php echo htmlspecialchars($r['img_path']); ?>" alt="ภาพประกอบ">
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
    <?php endif; ?>
...
document.getElementById('reportForm').addEventListener('submit', function(e){
    if(!document.getElementById('cat-input').value){
        e.preventDefault();
        showToast('warning', 'กรุณาเลือกหมวดหมู่ปัญหาก่อน');
        return;
    }
    if(!document.getElementById('prob-type-input').value){
        e.preventDefault();
        showToast('warning', 'กรุณาเลือกประเภทปัญหา');
        return;
    }

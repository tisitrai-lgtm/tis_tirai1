...
// --- เพิ่มส่วนการรับค่าปีการผลิตจาก Session ---
$selected_year = isset($_SESSION['selected_year']) ? $_SESSION['selected_year'] : '';
// ---------------------------------------

// *** สำคัญมาก: กำหนด BASE_IMAGE_ROOT_PATH
define('BASE_IMAGE_ROOT_PATH', ''); // <<< คุณต้องปรับค่านี้ให้ถูกต้องตามโครงสร้างโฟลเดอร์ของคุณ
...
<head>
    ...
    <style>
        ...
        /* สไตล์เพิ่มเติมสำหรับแสดงปีที่เลือก */
        .year-badge {
...
<div class="content-wrapper">
    <div class="container mt-5">
        ...
        <div class="text-center welcome-msg">
            ...
            <br>
            <span class="year-badge">ปีการผลิตที่เลือก: <?php echo htmlspecialchars($selected_year); ?></span>
        </div>
        ...
        <div class="table-responsive">
            <table id="dataTable" class="table table-striped table-bordered nowrap" style="width:100%">
                ...
                <tbody>
                    <?php
                    // --- ปรับ SQL ให้กรองตาม emp_id และ year_rai ที่เลือกมาจาก Login ---
                    $emp_id_clean = mysqli_real_escape_string($con, $_SESSION['emp_id']);
                    $year_clean = mysqli_real_escape_string($con, $selected_year);

                    $sql_data = "SELECT * FROM image_water 
                                 WHERE emp_id = '$emp_id_clean' 
                    ...
                    // --- แก้ไขจุดนี้: ทำเลขสัญญาให้เป็น 6 หลัก ---
                        ...
                        echo "<tr>
                            <td>
<a href='user_water_edit_data.php?plot_id=" . urlencode($row_data['plot_id']) . "&year_rai=" . urlencode($row_data['year_rai']) . "' class='btn btn-warning btn-sm'><i class='bx bx-edit-alt'></i></a>                            <td>" . htmlspecialchars($row_data['plot_id']) . "</td>
                        ...
                        foreach ($image_columns as $col_name => $alt_text) {
                            $image_file_name_from_db = $row_data[$col_name];
                            $full_image_path = '';

                            if (!empty($image_file_name_from_db)) {
                                $full_image_path = $base_image_folder_path . htmlspecialchars(basename($image_file_name_from_db));
                            }

                            echo "<td>";
                            if (!empty($full_image_path) && file_exists($full_image_path)) {
                                echo "<img src='" . htmlspecialchars($full_image_path) . "' class='img-thumbnail' alt='" . htmlspecialchars($alt_text) . "'>";
                            } else {
        ...
        $(document).ready(function() {
            $('#dataTable').DataTable({
                ...
                language: {
                    ...
                    zeroRecords: "ไม่พบข้อมูลของปีการผลิตที่เลือก", 
